...
static int GLimp_SetMode(int mode, qboolean fullscreen, qboolean noborder) {
	...
	if (videoInfo == NULL) {
		...
		if (videoInfo->current_h > 0) {
			// Guess the display aspect ratio through the desktop resolution by assuming(relatively safely)that it is set at or close to the display's native aspect ratio
...
#ifdef USE_ICON
		{
			SDL_Surface *icon = SDL_CreateRGBSurfaceFrom((void *)CLIENT_WINDOW_ICON.pixel_data, CLIENT_WINDOW_ICON.width, CLIENT_WINDOW_ICON.height, CLIENT_WINDOW_ICON.bytes_per_pixel * 8, CLIENT_WINDOW_ICON.bytes_per_pixel * CLIENT_WINDOW_ICON.width,
	...
	if (r_tvMode->integer > -1) {
		...
		if (glConfig.vidWidth > tvWidth) {
			glConfig.vidWidth = tvWidth; // clamp
		}
	
		if (glConfig.vidHeight > tvHeight) {
			glConfig.vidHeight = tvHeight; // clamp
		}
...
success:
	...
	// Mysteriously, if you use an NVidia graphics card and multiple monitors, SDL_SetGamma will incorrectly return false the first time; ask
	// again and you get the correct answer. This is a suspected driver bug, see http://bugzilla.icculus.org/show_bug.cgi?id=4316
	glConfig.deviceSupportsGamma = SDL_SetGamma(1.0f, 1.0f, 1.0f) >= 0;
#ifdef _WIN32
	// leilei - 3dfx gamma 
