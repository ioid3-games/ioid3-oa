...
static const char *IN_TranslateSDLToQ3Key(SDL_keysym *keysym,
	...
	*key = 0;
	...
	// Keys that have ASCII names but produce no character are probably dead keys -- ignore them
...
static void IN_ActivateMouse(void) {
	...
	if (!mouseActive) {
		...
		// This is a bug in the current SDL/macosx...have to toggle it a few times to get the cursor to hide.
		SDL_ShowCursor(1);
		SDL_ShowCursor(0);
#endif
