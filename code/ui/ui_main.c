...
// Changed RD
					...
					Text_PaintChar(x - 3, y - yadj - 3, glyph->imageWidth + 6, glyph->imageHeight + 6, useScale, glyph->s, glyph->t, glyph->s2, glyph->t2,´glyph->glyph);

					trap_R_SetColor(outer);
					Text_PaintChar(x - 1, y - yadj - 1, glyph->imageWidth + 2, glyph->imageHeight + 2, useScale, glyph->s, glyph->t, glyph->s2, glyph->t2, glyph->glyph);

					trap_R_SetColor(inner);
					Text_PaintChar(x - 0.5, y - yadj - 0.5, glyph->imageWidth + 1, glyph->imageHeight + 1, useScale, glyph->s, glyph->t, glyph->s2, glyph->t2, glyph->glyph);
...
// Changed RD
		...
		UI_PlayerInfo_SetInfo(&uiInfo.info, trap_Cvar_VariableValue("ui_LowerAnim"), trap_Cvar_VariableValue("ui_UpperAnim"), viewangles, moveangles, trap_Cvar_VariableValue("ui_Weapon"), qfalse);
// end changed RD
		updateModel = qfalse;
	}
...
static void UI_BinaryServerInsertion(int num) {
	...
	while (mid > 0) {
		mid = len >> 1;
		res = trap_LAN_CompareServers(ui_netSource.integer, uiInfo.serverStatus.sortKey, uiInfo.serverStatus.sortDir, num, uiInfo.serverStatus.displayServers[offset + mid]);
		// if equal
		if (res == 0) {
