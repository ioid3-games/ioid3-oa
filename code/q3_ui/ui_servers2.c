...
This function will return the real length of the string if numChars len of character data is desired. It looks for color codes and adds
2 to the length for each combo found. This is used to make color strings show up correctly in column formatted environments. Otherwise,
the columns will be off 2 * num of color codes.
=======================================================================================================================================
*/
...
static void ArenaServers_UpdateMenu(void) {
	...
	for (i = 0, j = 0; i < count; i++, servernodeptr++) {
		...
		/*
		Com_sprintf(buff, MAX_LISTBOXWIDTH, "%-20.20s %-12.12s %2d/%2d %-8.8s %3s %s%3d ", servernodeptr->hostname, servernodeptr->mapname, servernodeptr->numclients, servernodeptr->maxclients, servernodeptr->gamename, netnames[servernodeptr->nettype], pingColor, servernodeptr->pingtime, servernodeptr->bPB ? "Yes" : "No"
        */
        b = buff;
                *b++= '^';
                *b++= '7';
