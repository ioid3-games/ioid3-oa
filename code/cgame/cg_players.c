...
static qboolean CG_ScanForExistingClientInfo(clientInfo_t *ci) {
	...
	for (i = 0; i < cgs.maxclients; i++) {
		...
		if (Q_strequal(ci->modelName, match->modelName) && Q_strequal(ci->skinName, match->skinName) && Q_strequal(ci->headModelName, match->headModelName) && Q_strequal(ci->headSkinName, match->headSkinName) && Q_strequal(ci->blueTeam, match->blueTeam) && Q_strequal(ci->redTeam, match->redTeam) && (cgs.gametype < GT_TEAM || cgs.ffa_gt == 1 || ci->team == match->team)) {
...
static void CG_SetDeferredClientInfo(int clientNum, clientInfo_t *ci) {
	...
	for (i = 0; i < cgs.maxclients; i++) {
		...
		if (!Q_strequal(ci->skinName, match->skinName) || !Q_strequal(ci->modelName, match->modelName) || (cgs.gametype >= GT_TEAM && cgs.ffa_gt != 1 && ci->team != match->team)) {
			continue;
		}
	...
	if (cgs.gametype >= GT_TEAM && cgs.ffa_gt != 1) {
		for (i = 0; i < cgs.maxclients; i++) {
			...
			if (!Q_strequal(ci->skinName, match->skinName) || (cgs.gametype >= GT_TEAM && cgs.ffa_gt != 1 && ci->team != match->team)) {
				continue;
			}
