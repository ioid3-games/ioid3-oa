...
void CG_SmallBloodTrail(localEntity_t *le) {
	...
	for (; t <= t2; t += step) {
		...
		blood = CG_SmokePuff(newOrigin, vec3_origin, 3, 1, 1, 1, 1, 770, t, 0, 0, cgs.media.lbldShader1);
		// use the optimized version
		blood->leType = LE_FALL_SCALE_FADE;
		// drop a total of 40 units over its lifetime
...
void CG_GoreMark(localEntity_t *le, trace_t *trace) {
	...
	if (le->leMarkType == LEMT_BURN) {
		radius = 6 + (rand()&16);
		CG_ImpactMark(cgs.media.lbldShader2, trace->endpos, trace->plane.normal, random() * 360, 1, 1, 1, 1, qtrue, radius, qfalse);
...
void CG_AddGore(localEntity_t *le) {
	...
	// if it is in a nodrop zone, remove it this keeps gibs from waiting at the bottom of pits of death and floating levels
