...
/*static float CG_DrawDoubleDominationThings(float y) {
	...
	} else if (statusB == TEAM_FREE) {
		s = va("Point B is not controlled");
	} else if (statusB == TEAM_RED) {
		s = va("Point B is controlled by RED");
	} else if (statusB == TEAM_BLUE) {
...
static void CG_DrawFragMsgIcons(fragInfo_t fi, int i) {
	...
	if (fi.attackerName[0] != '\0') {
		CG_DrawStringExt(FRAGMSG_X, FRAGMSG_Y + (i * TINYCHAR_HEIGHT), fi.attackerName, hcolor, qfalse, qfalse, TINYCHAR_WIDTH, TINYCHAR_HEIGHT, 0);
		x_offset = CG_DrawStrlen(fi.attackerName) * TINYCHAR_WIDTH + spacing;
	} else {
		x_offset = 0;
	}

	CG_DrawPic(FRAGMSG_X + x_offset, FRAGMSG_Y + (i * TINYCHAR_HEIGHT), TINYCHAR_WIDTH, TINYCHAR_HEIGHT, fi.causeShader);

	x_offset += TINYCHAR_WIDTH + spacing;

	CG_DrawStringExt(FRAGMSG_X + x_offset, FRAGMSG_Y + (i * TINYCHAR_HEIGHT), fi.targetName, hcolor, qfalse, qfalse, TINYCHAR_WIDTH, TINYCHAR_HEIGHT, 0);

	if (fi.teamFrag) {
		x_offset += CG_DrawStrlen(fi.targetName) * TINYCHAR_WIDTH + spacing;
		CG_DrawStringExt(FRAGMSG_X + x_offset, FRAGMSG_Y + (i * TINYCHAR_HEIGHT), "(^1TEAMMATE^7)", hcolor, qfalse, qfalse, TINYCHAR_WIDTH, TINYCHAR_HEIGHT, 0);
	}
...
static void CG_DrawFragMsgText(char *msg, int i) {
	...
	trap_R_SetColor(color);
	CG_Text_Paint(FRAGMSG_X, FRAGMSG_Y + (i * 13), 0.20, color, msg, 0, 0, ITEM_TEXTSTYLE_SHADOWED);
#else
	hcolor[0] = hcolor[1] = hcolor[2] = hcolor[3] = 1.0;
	trap_R_SetColor(hcolor);

	CG_DrawStringExt(FRAGMSG_X, FRAGMSG_Y + (i * TINYCHAR_HEIGHT), msg, hcolor, qfalse, qfalse, TINYCHAR_WIDTH, TINYCHAR_HEIGHT, 0);
#endif
	trap_R_SetColor(NULL);
}
	...
	for (counter = 0; counter < WP_NUM_WEAPONS; counter++) {
		if (cg_weapons[counter + 2].weaponIcon && counter != WP_PROX_LAUNCHER && counter != WP_GRAPPLING_HOOK) {
			...
			if (cg.accuracys[counter][0] > 0) {
				CG_DrawSmallStringColor(ACCBOARD_XPOS + 10 + ACCITEM_SIZE + 10, ACCBOARD_YPOS + 10 + i * ACCBOARD_HEIGHT + ACCITEM_SIZE / 2 - SMALLCHAR_HEIGHT / 2, va("%i%s", (int)(((float)cg.accuracys[counter][1] * 100) / ((float)(cg.accuracys[counter][0]))), "%"), colorWhite);
			} else {
