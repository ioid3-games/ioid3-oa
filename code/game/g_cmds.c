...
void EliminationMessage(gentity_t *ent) {
	trap_SendServerCommand(ent - g_entities, va("elimination %i %i %i", level.teamScores[TEAM_RED], level.teamScores[TEAM_BLUE], level.roundStartTime));
}
...
void SetTeam(gentity_t *ent, char *s) {
	...
	if (!force) {
		if (team == TEAM_RED && level.RedTeamLocked) {
			trap_SendServerCommand(ent->client->ps.clientNum, "cp \"The Red Team has been locked by the Admin!\n\"");
			return;
		}

		if (team == TEAM_BLUE && level.BlueTeamLocked) {
			trap_SendServerCommand(ent->client->ps.clientNum, "cp \"The Blue Team has been locked by the Admin!\n\"");
			return;
		}

		if (team == TEAM_FREE && level.FFALocked) {
			trap_SendServerCommand(ent->client->ps.clientNum, "cp \"This Deathmatch has been locked by the Admin!\n\"");
			return;
		}
	}
...
void G_Say(gentity_t *ent, gentity_t *target, int mode, const char *chatText) {
	...
	switch (mode) {
		...
		case SAY_TEAM:
			...
			break;
		case SAY_TELL:
			if (target && target->inuse && target->client && g_gametype.integer >= GT_TEAM && g_ffa_gt != 1 && target->client->sess.sessionTeam == ent->client->sess.sessionTeam && Team_GetLocationMsg(ent, location, sizeof(location))) {
				Com_sprintf(name, sizeof(name), EC"[%s%c%c"EC"] (%s)"EC": ", ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE, location);
			} else {
				Com_sprintf(name, sizeof(name), EC"[%s%c%c"EC"]"EC": ", ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE);
			}
...
static void Cmd_Voice_f(gentity_t *ent) {
	...
	if ((Q_strequal(arg, "vsay_team")) || Q_strequal(arg, "vosay_team")) {
		mode = SAY_TEAM;
	}
...
void ClientCommand(int clientNum) {
	...
	if ((cmds[i].cmdFlags & CMD_NOTEAM ||
		(cmds[i].cmdFlags & CMD_CHEAT_TEAM && !g_cheats.integer)) && ent->client->sess.sessionTeam != TEAM_NONE) {
		trap_SendServerCommand(clientNum, "print \"Cannot use this command when on a team\n\"");
		return;
	}

	if (cmds[i].cmdFlags & CMD_RED && ent->client->sess.sessionTeam != TEAM_RED) {
		trap_SendServerCommand(clientNum, "print \"Must be on the Red Team to use this command\n\"");
		return;
	}

	if (cmds[i].cmdFlags & CMD_BLUE && ent->client->sess.sessionTeam != TEAM_BLUE) {
		trap_SendServerCommand(clientNum, "print \"Must be on the Blue Team to use this command\n\"");
		return;
	}
