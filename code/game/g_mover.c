...
qboolean G_MoverPush(gentity_t *pusher, vec3_t move, vec3_t amove, gentity_t **obstacle) {
	...
	*obstacle = NULL;
	...
	for (e = 0; e < listedEntities; e++) {
		...
		// bobbing entities are instant-kill and never get blocked, and so are train entities
...
When a button is touched, it moves some distance in the direction of its angle, triggers all of its targets, waits some time, then returns to it's original position where it can be triggered again.
