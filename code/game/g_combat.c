...
void TossClientItems(gentity_t *self) {
	...
	// make a special check to see if they are changing to a new weapon that isn't the mg or gauntlet.  Without this, a client
...
void G_Damage(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker, vec3_t dir, vec3_t point, int damage, int dflags, int mod) {
	...
	if ((attacker == &g_entities[ENTITYNUM_WORLD] || attacker == targ || attacker->s.eType != ET_PLAYER) && client && client->lastSentFlying > - 1 && (mod == MOD_FALLING || mod == MOD_LAVA || mod == MOD_SLIME || mod == MOD_TRIGGER_HURT || mod == MOD_SUICIDE)) {
	...
	if (g_vampire.value > 0.0 && (targ != attacker) && take > 0 && !(OnSameTeam(targ, attacker)) && attacker->health > 0 && targ->health > 0) {
		if (take < targ->health) {
