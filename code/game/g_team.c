...
Open Arena source code is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as
published by the Free Software Foundation; either version 2 of the License, or (at your option) any later version.

Open Arena source code is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty
of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License for more details.

You should have received a copy of the GNU General Public License along with Open Arena source code; if not, write to the Free Software
...
void Team_DD_bonusAtPoints(int team) {
	...
	for (i = 0; i < MAX_CLIENTS; i++) {
		...
		if (!(((VectorLength(v1) < CTF_TARGET_PROTECT_RADIUS && trap_InPVS(ddA->r.currentOrigin, player->r.currentOrigin)) || (VectorLength(v2) < CTF_TARGET_PROTECT_RADIUS && trap_InPVS(ddB->r.currentOrigin, player->r.currentOrigin))))) {
...
void Team_FragBonuses(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker) {
	...
	if (((VectorLength(v1) < CTF_TARGET_PROTECT_RADIUS && trap_InPVS(flag->r.currentOrigin, targ->r.currentOrigin)) || (VectorLength(v2) < CTF_TARGET_PROTECT_RADIUS && trap_InPVS(flag->r.currentOrigin, attacker->r.currentOrigin))) &&
		attacker->client->sess.sessionTeam != targ->client->sess.sessionTeam && g_gametype.integer != GT_ELIMINATION && (g_gametype.integer != GT_CTF_ELIMINATION || !g_elimination_ctf_oneway.integer ||
...
int Pickup_Team(gentity_t *ent, gentity_t *other) {
	...
	if (g_gametype.integer == GT_HARVESTER) {
		...
		if (ent->spawnflags != cl->sess.sessionTeam) {
			...
			G_LogPrintf("HARVESTER: %i %i %i %i %i: %s picked up a skull.\n", cl->ps.clientNum, cl->sess.sessionTeam, 3, -1, 1, cl->pers.netname);
		} else {
