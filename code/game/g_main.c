...
#else {&g_persistantpowerups, "g_runes", "0", CVAR_LATCH|CVAR_ARCHIVE, 0, qfalse},
#endif
...
static int G_UpdateTimestamp(void) {
	...
	trap_Cvar_Set("g_timestamp", va("%04i - %02i - %02i %02i:%02i:%02i", 1900 + timestamp.tm_year, 1 + timestamp.tm_mon, timestamp.tm_mday, timestamp.tm_hour, timestamp.tm_min, timestamp.tm_sec));
	return ret;
}
...
Runs a script of it exists. The script that is to be exec'ed.
0 if the script ran, 1 if it did not exist.
=======================================================================================================================================
*/
...
qboolean ScoreIsTied(void) {
	...
	if (g_gametype.integer == GT_ELIMINATION || (g_gametype.integer == GT_CTF_ELIMINATION && g_elimination_ctf_oneway.integer)) {
		return (level.teamScores[TEAM_RED] == level.teamScores[TEAM_BLUE] || level.teamScores[TEAM_RED] == level.teamScores[TEAM_BLUE] + 1 || level.teamScores[TEAM_RED] == level.teamScores[TEAM_BLUE] - 1);
	}
