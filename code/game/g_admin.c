		...
		if (Q_strequal(ent->client->pers.guid, g_admin_admins[i]->guid)) {
			...
			while (*flags) {
				if (*flags == flag) {
					...
					// flags with significance only for individuals(like ADMF_INCOGNITO and ADMF_IMMUTABLE are NOT covered by the '*' wildcard.  They must be specified manually.
...
static void admin_writeconfig(void) {
	...
	if (!g_admin.string[0]) {
		G_Printf(S_COLOR_YELLOW "WARNING: g_admin is not set. Configuration will not be saved to a file.\n");
		return;
	}
	...
	for (i = 0; i < MAX_ADMIN_BANS && g_admin_bans[i]; i++) {
		// don't write expired bans if expires is 0, then it's a perm ban
	...
	for (i = 0; i < MAX_ADMIN_WARNINGS && g_admin_warnings[i]; i++) {
		...
		if (g_admin_warnings[i]->expires != 0 && (g_admin_warnings[i]->expires - t) < 1) {
			continue;
		}
...
static void admin_log(gentity_t *admin, char *cmd, int skiparg) {
	...
	*flags = '\0';
	...
	if (victim && !Q_strequal(cmd, "attempted")) {
		Com_sprintf(string, sizeof(string), "%3i:%i%i: %i: %s: %s: %s: %s: %s: %s: \"%s\"\n", min, tens, sec, (admin) ? admin->s.clientNum : - 1, (admin) ? admin->client->pers.guid : "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX", (admin) ? admin->client->pers.netname : "console", flags, cmd, victim->client->pers.guid, victim->client->pers.netname, G_SayConcatArgs(2 + skiparg));
	} else {
		Com_sprintf(string, sizeof(string), "%3i:%i%i: %i: %s: %s: %s: %s: \"%s\"\n", min, tens, sec, (admin) ? admin->s.clientNum : - 1, (admin) ? admin->client->pers.guid : "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX", (admin) ? admin->client->pers.netname : "console", flags, cmd, G_SayConcatArgs(1 + skiparg));
	}
...
static int admin_listadmins(gentity_t *ent, int start, char *search) {
	...
	for (i = start; i < MAX_ADMIN_ADMINS && g_admin_admins[i] && drawn < MAX_ADMIN_LISTITEMS; i++) {
		if (search[0]) {
			...
			// verify we don't have the same guid / name pair in connected players since we don't want to draw the same player twice
			dup = qfalse;
			...
			for (j = 0; j < level.maxclients; j++) {
				...
				if (Q_strequal(vic->client->pers.guid, g_admin_admins[i]->guid) && strstr(name2, search)) {
					dup = qtrue;
					break;
				}
			}
		...
		ADMBP(va("%4i %4i %s^7(*%s) %s^7\n", (i + MAX_CLIENTS), g_admin_admins[i]->level, lname, guid_stub, g_admin_admins[i]->name));
		drawn++;
	}
...
qboolean G_admin_cmd_check(gentity_t *ent, qboolean say) {
	...
	if (Q_strequal(command, "say") || (Q_strequal(command, "say_team") && G_admin_permission(ent, ADMF_TEAMCHAT_CMD))) {
		skip = 1;
...
void G_admin_namelog_update(gclient_t *client, qboolean disconnect) {
	...
	for (i = 0; i < MAX_ADMIN_NAMELOGS && g_admin_namelog[i]; i++) {
		...
		if (Q_strequal(client->pers.ip, g_admin_namelog[i]->ip) && Q_strequal(client->pers.guid, g_admin_namelog[i]->guid)) {
...
qboolean G_admin_setlevel(gentity_t *ent, int skiparg) {
	...
	// if admin is activated for the first time on a running server, we need to ensure at least the default levels get created
	...
	if (matches == 0) {
		ADMP("^3!setlevel:^7 no match.  use !listplayers or !listadmins to find an appropriate number to use instead of name.\n");
		return qfalse;
	}

	if (matches > 1) {
		ADMP("^3!setlevel:^7 more than one match.  Use the admin number instead:\n");
...
=======================================================================================================================================
*/
static qboolean admin_create_ban(gentity_t *ent, char *netname, char *guid, char *ip, int seconds, char *reason) {
	g_admin_ban_t *b = NULL;
	qtime_t qt;
	int t;
	int i;
...
static qboolean admin_create_warning(gentity_t *ent, char *netname, char *guid, char *ip, int seconds, char *warning) {
	...
	// strftime(b->made, sizeof(b->made), "%m / %d / %y %H:%M:%S", lt);
	Com_sprintf(w->made, sizeof(w->made), "%02i / %02i / %02i %02i:%02i:%02i", qt.tm_mon + 1, qt.tm_mday, qt.tm_year % 100, qt.tm_hour, qt.tm_min, qt.tm_sec);
...
qboolean G_admin_kick(gentity_t *ent, int skiparg) {
	...
	admin_create_ban(ent, vic->client->pers.netname, vic->client->pers.guid, vic->client->pers.ip, G_admin_parse_time(va("1s%s", g_adminTempBan.string)), (*reason) ? reason : "kicked by admin");
	...
	trap_DropClient(pids[0], va("has been kicked%s^7. reason: %s", (ent) ? va(" by %s", ent->client->pers.netname) : "", (*reason) ? reason : "kicked by admin"));
...
qboolean G_admin_ban(gentity_t *ent, int skiparg) {
	...
	if (G_admin_permission(ent, ADMF_CAN_PERM_BAN) && G_admin_permission(ent, ADMF_UNACCOUNTABLE)) {
		minargc = 2 + skiparg;
	} else if (G_admin_permission(ent, ADMF_CAN_PERM_BAN) || G_admin_permission(ent, ADMF_UNACCOUNTABLE) || g_adminMaxBan.integer) {
		minargc = 3 + skiparg;
	} else {
		minargc = 4 + skiparg;
	}
	...
	if (seconds <= 0) {
		...
		if (g_adminMaxBan.integer && !G_admin_permission(ent, ADMF_CAN_PERM_BAN) && seconds > G_admin_parse_time(g_adminMaxBan.string)) {
		...
		for (i = 0; i < MAX_ADMIN_NAMELOGS && g_admin_namelog[i]; i++) {
			...
			for (j = 0; j < MAX_ADMIN_NAMELOG_NAMES && g_admin_namelog[i]->name[j][0]; j++) {
				...
				if (strstr(n2, s2)) {
					...
					ADMBP(va("% - 2s(*%s) %15s ^7'%s^7'\n", (g_admin_namelog[i]->slot > - 1) ? va("%d", g_admin_namelog[i]->slot) : " - ", guid_stub, g_admin_namelog[i]->ip, g_admin_namelog[i]->name[j]));
				}
			}
		}
	...
	if (g_admin_namelog[logmatch]->slot == -1) {
		...
		AP(va("print \"^3!ban:^7 %s^7 has been banned by %s^7, duration: %s, reason: %s\n\"", g_admin_namelog[logmatch]->name[0], (ent) ? ent->client->pers.netname : "console", duration, (*reason) ? reason : "banned by admin"));
		return qtrue;
	}

	trap_SendServerCommand(g_admin_namelog[logmatch]->slot, va("disconnect \"You have been banned.\n admin:\n%s^7\nduration:\n%s\nreason:\n%s\"", (ent) ? ent->client->pers.netname : "console", duration, (*reason) ? reason : "banned by admin"));

	trap_DropClient(g_admin_namelog[logmatch]->slot, va("has been banned by %s^7, duration: %s, reason: %s", (ent) ? ent->client->pers.netname : "console", duration, (*reason) ? reason : "banned by admin"));
	return qtrue;
}
...
qboolean G_admin_adjustban(gentity_t *ent, int skiparg) {
	...
	if (g_adminMaxBan.integer && !G_admin_permission(ent, ADMF_CAN_PERM_BAN) && ban->expires - time > G_admin_parse_time(g_adminMaxBan.string)) {
	...
	AP(va("print \"^3!adjustban: ^7ban #%d for %s^7 has been updated by %s^7 %s%s%s%s%s\n\"", bnum, ban->name, (ent) ? ent->client->pers.netname : "console", (length >= 0) ? "duration: " : "", duration, (length >= 0 && *reason) ? ", " : "", (*reason) ? "reason: " : "", reason));
...
qboolean G_admin_putteam(gentity_t *ent, int skiparg) {
	...
	AP(va("print \"^3!putteam: ^7%s^7 put %s^7 on to the %s team\n\"", (ent) ? ent->client->pers.netname : "console", vic->client->pers.netname, BG_TeamName(teamnum)));
	return qtrue;
}
...
qboolean G_admin_listadmins(gentity_t *ent, int skiparg) {
	...
	if (search[0]) {
		...
		ADMBP_begin();
		ADMBP(va("^3!listadmins:^7 showing admin %d - %d of %d.  ", (found) ?(start + 1) : 0, ((start + MAX_ADMIN_LISTITEMS) > found) ? found :(start + MAX_ADMIN_LISTITEMS), found));

		if ((start + MAX_ADMIN_LISTITEMS) < found) {
			ADMBP(va("run '!listadmins %d' to see more", (start + MAX_ADMIN_LISTITEMS + 1)));
		}
...
qboolean G_admin_listplayers(gentity_t *ent, int skiparg) {
	...
	for (i = 0; i < level.maxclients; i++) {
		...
		ADMBP(va("%2i %s%s^7 % - 2i %s^7(*%s) ^1%1s^7 %s^7 %s%s^7%s\n", i, c, t, l, lname, guid_stub, muted, p->pers.netname, (*n) ? "(a.k.a. " : "", n, (*n) ? ")" : ""));
	}
...
qboolean G_admin_showbans(gentity_t *ent, int skiparg) {
	...
	if (G_SayArgc() >= 2 + skiparg) {
		...
		for (i = 0; i < sizeof(filter) && filter[i]; i++) {
			if (!isdigit(filter[i]) && filter[i] != '.' && filter[i] != '-') {
				numeric = qfalse;
				break;
			}
		}
	...
	for (i = start, count = 0; i <= max && count < MAX_ADMIN_SHOWBANS; i++) {
		...
		if (ip_match && Q_strncmp(ip_match, g_admin_bans[i]->ip, ip_match_len)) {
			continue;
		}
	...
	if (name_match[0] || ip_match) {
		ADMBP(va("^3!showbans:^7 found %d matching bans by %s.  ", count, (ip_match) ? "IP" : "name"));
	} else {
		ADMBP(va("^3!showbans:^7 showing bans %d - %d of %d(%d total).", (found) ?(start + 1) : 0, i, max + 1, found));
	}

	if (i <= max) {
		ADMBP(va("  run !showbans %d%s%s to see more", i + 1, (filter[0]) ? " " : "", (filter[0]) ? filter : ""));
	}
...
qboolean G_admin_admintest(gentity_t *ent, int skiparg) {
	...
	AP(va("print \"^3!admintest: ^7%s^7 is a level %d admin %s%s^7%s\n\"", ent->client->pers.netname, l, (lname) ? "(" : "", (lname) ? g_admin_levels[i]->name : "", (lname) ? ")" : ""));
	return qtrue;
}
...
qboolean G_admin_namelog(gentity_t *ent, int skiparg) {
	...
	for (i = 0; i < MAX_ADMIN_NAMELOGS && g_admin_namelog[i]; i++) {
		...
		ADMBP(va("% - 2s(*%s) %15s^7", (g_admin_namelog[i]->slot > - 1) ? va("%d", g_admin_namelog[i]->slot) : " - ", guid_stub, g_admin_namelog[i]->ip));
...
qboolean G_admin_lock(gentity_t *ent, int skiparg) {
	...
	AP(va("print \"^3!lock: ^7the %s team has been locked by %s\n\"", BG_TeamName(team), (ent) ? ent->client->pers.netname : "console"));
	return qtrue;
}
...
qboolean G_admin_disorient(gentity_t *ent, int skiparg) {
	...
	if (!(vic->client->sess.sessionTeam == TEAM_RED || vic->client->sess.sessionTeam == TEAM_BLUE || vic->client->sess.sessionTeam == TEAM_FREE)) {
		ADMP("^ / disorient: ^7player must be on a team");
		return qfalse;
	}

	if (vic->client->pers.disoriented) {
		ADMP(va("^ / disorient: ^7%s^7 is already disoriented", vic->client->pers.netname));
		return qfalse;
	}

	vic->client->pers.disoriented = qtrue;

	AP(va("chat \"^ / disorient: ^7%s ^7is disoriented\" - 1", vic->client->pers.netname));
	CPx(pids[0], va("cp \"%s ^7disoriented you%s%s\"", (ent?ent->client->pers.netname:"^3SERVER CONSOLE"), (*reason) ? " because:\n" : "", (*reason) ? reason : ""));
	return qtrue;
}
...
qboolean G_admin_orient(gentity_t *ent, int skiparg) {
	...
	if (!vic->client->pers.disoriented) {
		ADMP(va("^ / orient: ^7%s^7 is not currently disoriented", vic->client->pers.netname));
		return qfalse;
	}
	...
	CPx(pids[0], va("cp \"%s ^7oriented you\"", (ent?ent->client->pers.netname:"^3SERVER CONSOLE")));
	return qtrue;
}
...
qboolean G_admin_slap(gentity_t *ent, int skiparg) {
	...
	CPx(pids[0], va("cp \"%s ^7slapped you%s%s\"", (ent?ent->client->pers.netname:"^3SERVER CONSOLE"), (*reason) ? " because:\n" : "", (*reason) ? reason : ""));
	return qtrue;
}
...
qboolean G_admin_warn(gentity_t *ent, int skiparg) {
	...
	admin_create_warning(ent, vic->client->pers.netname, vic->client->pers.guid, vic->client->pers.ip, seconds, reason);
	...
	if (g_maxWarnings.integer) {
		...
		if (totalWarnings >= g_maxWarnings.integer) {
			...
			admin_create_ban(ent, vic->client->pers.netname, vic->client->pers.guid, vic->client->pers.ip, seconds, "Too Many Warnings");

			if (g_admin.string[0]) {
				admin_writeconfig();
			}

			trap_SendServerCommand(pids[0], va("disconnect \"You have been kicked.\n%s^7\nreason:\n%s\"", (ent) ? va("admin:\n%s", ent->client->pers.netname) : "SERVER", "Too Many Warnings"));
			trap_DropClient(pids[0], va("has been kicked%s^7. reason: %s", "Auto - Admin System", "Too Many Warnings"));
			return qtrue;
		} else {
			// Print it to everybody
			AP(va("chat \"^ / warn: ^7%s ^7was warned\" - 1", vic->client->pers.netname));
			// CenterPrint it to the Person Being Slapped
			CPx(pids[0], va("cp \"%s ^7warned you%s%s\"", (ent?ent->client->pers.netname:"^3SERVER CONSOLE"), (*reason) ? " because:\n" : "", (*reason) ? reason : ""));
			return qtrue;
		}
...
This function facilitates the ADMP define. ADMP() is similar to CP except that it prints the message to the server console if ent is
not defined.
=======================================================================================================================================
*/
