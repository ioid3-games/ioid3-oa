...
void BotCTFOrders_FlagNotAtBase(bot_state_t *bs) {
	...
	numteammates = BotSortTeamMatesByBaseTravelTime(bs, teammates, sizeof(teammates));
	BotSortTeamMatesByTaskPreference(bs, teammates, numteammates);
	// in oneway ctf we must all move out of the base (only one strategy, maybe we can also send some to the enemy base to meet the flag carier?)
...
void BotTeamAI(bot_state_t *bs) {
	...
	switch (gametype) {
		...
		case GT_DOUBLE_D:
		{
			// if the number of team mates changed or the domination point status changed or someone wants to know what to do
