...
Pick a random info_player_deathmatch spawnpoint. Will prefer a spot that wont telefrag if such a point exist. 
filter_flags Will NEVER pick a spawnpoint mathcing these flags.
Returns a pointer to a spot or NULL if no spots exists.
=======================================================================================================================================
*/
#define MAX_SPAWN_POINTS 128
...
// unlagged - client options
	...
	if (((client->sess.sessionTeam == TEAM_SPECTATOR) || (((client->isEliminated) /*||
		...
		(g_gametype.integer == GT_ELIMINATION || g_gametype.integer == GT_CTF_ELIMINATION || g_gametype.integer == GT_LMS))) && (client->sess.spectatorState == SPECTATOR_SCOREBOARD)) {
...
// this means this client is not running in our current process
	if (!isBot && !strequals(value, "localhost")) {
		...
		if (g_password.string[0] && !Q_strequal(g_password.string, "none") && !strequals(g_password.string, value)) {
...
void ClientBegin(int clientNum) {
	...
	// save eflags around this, because changing teams will cause this to happen with a valid entity, and we want to make sure the teleport bit is set right
	// so the viewpoint doesn't interpolate through the world to the new position
	...
	if ((client->sess.sessionTeam != TEAM_SPECTATOR) && ((!(client->isEliminated) /*&&
		((!client->ps.pm_type) == PM_SPECTATOR)*/) ||// Sago: Yes, it made no sense ((g_gametype.integer != GT_ELIMINATION || level.intermissiontime) &&
		(g_gametype.integer != GT_CTF_ELIMINATION || level.intermissiontime) && (g_gametype.integer != GT_LMS || level.intermissiontime)))) {
		// send event
		tent = G_TempEntity(ent->client->ps.origin, EV_PLAYER_TELEPORT_IN);
		tent->s.clientNum = ent->s.clientNum;
...
//	char *savedAreaBits;
	...
	if (((g_gametype.integer == GT_ELIMINATION || g_gametype.integer == GT_CTF_ELIMINATION || (g_gametype.integer == GT_LMS && client->isEliminated)) && (!level.intermissiontime || level.warmupTime != 0)) && (client->sess.sessionTeam != TEAM_SPECTATOR)) {
		...
		if ((level.roundNumber == level.roundNumberStarted) || ((level.time < level.roundStartTime - g_elimination_activewarmup.integer * 1000) && TeamCount( - 1, TEAM_BLUE) && TeamCount( - 1, TEAM_RED))) {
	...
	if ((client->sess.sessionTeam == TEAM_SPECTATOR) || ((client->ps.pm_type == PM_SPECTATOR || client->isEliminated) && (g_gametype.integer == GT_ELIMINATION || g_gametype.integer == GT_CTF_ELIMINATION))) {
		...
		if (!client->pers.initialSpawn && client->pers.localClient) {
			...
			spawnPoint = SelectSpawnPoint(client->ps.origin, spawn_origin, spawn_angles, filters);
		}
	}
...
//	client->areabits = savedAreaBits;
	...
	if ((ent->client->sess.sessionTeam == TEAM_SPECTATOR) || ((client->ps.pm_type == PM_SPECTATOR || client->isEliminated) && (g_gametype.integer == GT_ELIMINATION || g_gametype.integer == GT_CTF_ELIMINATION || g_gametype.integer == GT_LMS))) {
